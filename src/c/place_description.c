...
void update_place_partial(struct place_descrition *place_d, Tuple* x_t, Tuple* y_t){
  ...
  APP_LOG(APP_LOG_LEVEL_DEBUG, "Place %s location updated: x: %d, y: %d", place_d->place_name, place_d->x, place_d->y); 
}
...
void update_place(struct place_descrition *place_d, Tuple *city_t, Tuple *offset_t, Tuple* x_t, Tuple* y_t)
{
  ...
  APP_LOG(APP_LOG_LEVEL_DEBUG, "Place changed to %s: x: %d, y: %d", place_d->place_name, place_d->x, place_d->y); 
}
